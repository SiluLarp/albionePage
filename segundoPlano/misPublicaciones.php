<?php
//se encargade cargar la lista de publicaciones de la cuenta del usuario
//cuenta.php
//funcion loadMiList
require "../conexion.php";
//recibimos la id del usuario
$nombreUsuario = $_POST['useron'];
$sql = mysqli_query($conexion,"SELECT id FROM usuario WHERE nombre = '$nombreUsuario'");
		$valor = mysqli_fetch_assoc($sql);
		$idSesion = $valor['id'];

$resultadoTDpost = mysqli_query($conexion, "SELECT * FROM publicacion WHERE id_usuario = '$idSesion'");//seleccionamos todos los reguistros de la tabla post
...
<div class="listaPublicacion">
	<div><img src="<?php echo $imagenVenta;?>" style="width: 60px;"></div>
	<table>
		<tr>
		<td colspan="2"><label style="font-size: 12px;"><?php echo $fila['item_name'];?></label></td>
		 </tr>
		 <tr>
...
<img src="img/prohibidoNegocio.png" class="imgTamaño" title="No negociable" style="display: <?php echo $visible2;?>;">
		...
		<div><img src="img/basura.png" style="width: 40px;cursor: pointer;" title="Borrar publicación" onclick="eliminarPublicacion('<?php echo $fila['id'] . "','../" . $imgBD;?>')"><br><br><img src="img/categorias/<?php echo $categoriaBD['nombre'];?>.png" class="imgtamacatego" title="<?php echo $categoriaBD['nombre'];?>"></div>
</div>
	<?php
}
...
echo "<div id='thereNotPost'><label>No tienes publicaciones</label></div>";
